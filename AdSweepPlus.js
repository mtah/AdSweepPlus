...
function adsweep(){
		if(location.href){
			window.adsweepVersion='1.6.17.2';
...
function adsweep_core(){
	...
	// Check if user-defined settings	
	try{
		if(myAdSweepWhitelist){
			if((typeof myAdSweepWhitelist)=="object"){
				whitelist=whitelist.concat(myAdSweepWhitelist);
			}
		}
		if(myAdSweepWhitelistRemove){
			if((typeof myAdSweepWhitelistRemove)=="object"){
				for(i in whitelist){
					for(a in myAdSweepWhitelistRemove){
						if(whitelist[i]==myAdSweepWhitelistRemove[a]){
		...
		if(myAdSweepHeuristicsExceptions){
			if((typeof myAdSweepHeuristicsExceptions)=="object"){
				adPatternExceptions=adPatternExceptions.concat(myAdSweepHeuristicsExceptions);
			}
		}
		if(myAdSweepURLreleaseList){
			if((typeof myAdSweepURLreleaseList)=="object"){
				for(i in adPatternExceptions){
					if((typeof adPatternExceptions[i])=="string"){
						for(a in myAdSweepURLreleaseList){
							if((typeof myAdSweepURLreleaseList[a])=="string"){
								if(adPatternExceptions[i]==myAdSweepURLreleaseList[a]){
									adPatternExceptions.splice(i,1);
								}
							}
						}
					}
		...
		if(MyAdSweepSites){
			if((typeof MyAdSweepSites)=="object"){
				for(a in MyAdSweepSites){
					sites[a]=MyAdSweepSites[a];
				}
			}
		}
		if(MyAdSweepCssExceptions){
			if((typeof MyAdSweepCssExceptions)=="object"){
				for(a in MyAdSweepCssExceptions){
					css_exceptions[a]=MyAdSweepCssExceptions[a];
				}
			}
		}
		if(MyAdSweepExtraCommonAdPatterns){
			if((typeof MyAdSweepExtraCommonAdPatterns)=="string"){
				common_ad_patterns+=','+MyAdSweepExtraCommonAdPatterns;
			}
		}
		if(MyAdSweepCustomCss){
			if((typeof MyAdSweepCustomCss)=="object"){
				for(a in MyAdSweepCustomCss){
					sites_custom_css[a]=MyAdSweepCustomCss[a];
				}
			}
		}
		if(myAdSweepScriptSrcMatchExceptions){
			if((typeof myAdSweepScriptSrcMatchExceptions)=="object"){
				for(i in scriptSrcMatchExceptions){
					for(a in myAdSweepScriptSrcMatchExceptions){
						if(scriptSrcMatchExceptions[i]==myAdSweepScriptSrcMatchExceptions[a]){
							scriptSrcMatchExceptions.splice(i,1);
						}
					}
				}
			}
		}
		if(myAdSweepScriptTxtMatchExceptions){
			if((typeof myAdSweepScriptTxtMatchExceptions)=="object"){
				for(i in scriptTxtMatchExceptions){
					for(a in myAdSweepScriptTxtMatchExceptions){
						if(scriptTxtMatchExceptions[i]==myAdSweepScriptTxtMatchExceptions[a]){
							scriptTxtMatchExceptions.splice(i,1);
						}
					}
				}
			}
		...
		if(MyAdSweepCommonAdPatternsExceptions){
			if((typeof MyAdSweepCommonAdPatternsExceptions)=="string"){
				MyAdSweepCommonAdPatternsExceptions = MyAdSweepCommonAdPatternsExceptions.split(',');
				for(var i in MyAdSweepCommonAdPatternsExceptions){
							if(common_ad_patterns.match(MyAdSweepCommonAdPatternsExceptions[i])){
								common_ad_patterns = removeCSSSelector(common_ad_patterns, MyAdSweepCommonAdPatternsExceptions[i]);
							}
				}
			}
		}
...
// Remove elements of the page using Javascript once the text of the page is loaded
function adsweep_removeAdNodes(){
	if(whitelistFlag==false){
		// Hide content using Javascript only if the page isn't on the exception list
		if(hide_common_ads==true){
		...
		if(URL.match("my.opera.com/community/forums")){if(document.getElementsByClassName('fpost')){var posts = document.getElementsByClassName('fpost');for(var a=0;a<posts.length;a++){if(posts[a].innerHTML.match("882703")){document.getElementById('content').removeChild(posts[a]);}}}}
...
function adsweep_installCheck(){if((URL.match(/^http:\/\/(www.)?adsweep.org\/$/))){document.getElementsByTagName("BODY")[0].innerHTML += '<div style="position:absolute;top:0;right:0;background:#c00;color:#fff;display:inline;padding:2px 5px">Your AdSweep is currently active (v.'+adsweepVersion+')</div>';}}
